...
#define DT_DRV_COMPAT bosch_bmi270plus
...
#include "bmi270plus_config_file.h"
...
int bmi270_reg_write_with_delay(
    const struct device *dev, uint8_t reg, const uint8_t *data, uint16_t length, uint32_t delay_us) {
    int ret = 0;
...
// mock function that reads step counter
static int bmi270_read_step_counter(uint16_t *step_cnt) {
    *step_cnt = 0;
...
static int set_accel_odr_osr(const struct device *dev, const struct sensor_value *odr, const struct sensor_value *osr) {
    ...
    if (odr) {
        ...
        if (odr_bits >= BMI270_ACC_ODR_100_HZ) {
            acc_conf = BMI270_SET_BITS(acc_conf, BMI270_ACC_FILT, BMI270_ACC_FILT_PERF_OPT);
        } else {
            acc_conf = BMI270_SET_BITS(acc_conf, BMI270_ACC_FILT, BMI270_ACC_FILT_PWR_OPT);
        }
    ...
    if (odr || osr) {
        ...
        ret = bmi270_reg_write_with_delay(dev, BMI270_REG_PWR_CTRL, &pwr_ctrl, 1, BMI270_INTER_WRITE_DELAY_US);
    }
...
static int set_accel_range(const struct device *dev, const struct sensor_value *range) {
    ...
    ret = bmi270_reg_write_with_delay(dev, BMI270_REG_ACC_RANGE, &acc_range, 1, BMI270_INTER_WRITE_DELAY_US);
...
static int set_gyro_odr_osr(const struct device *dev, const struct sensor_value *odr, const struct sensor_value *osr) {
    struct bmi270_data *data = dev->data;
    ...
    if (odr) {
        ...
        if (odr_bits >= BMI270_GYR_ODR_100_HZ) {
            gyr_conf = BMI270_SET_BITS(gyr_conf, BMI270_GYR_FILT, BMI270_GYR_FILT_PERF_OPT);
            gyr_conf = BMI270_SET_BITS(gyr_conf, BMI270_GYR_FILT_NOISE, BMI270_GYR_FILT_NOISE_PERF);
        } else {
            gyr_conf = BMI270_SET_BITS(gyr_conf, BMI270_GYR_FILT, BMI270_GYR_FILT_PWR_OPT);
            gyr_conf = BMI270_SET_BITS(gyr_conf, BMI270_GYR_FILT_NOISE, BMI270_GYR_FILT_NOISE_PWR);
        }
...
static int reset_step_counter(const struct device *dev) {
    struct bmi270_data *data = dev->data;
...
static int set_gyro_range(const struct device *dev, const struct sensor_value *range) {
    ...
    ret = bmi270_reg_write_with_delay(dev, BMI270_REG_GYR_RANGE, &gyr_range, 1, BMI270_INTER_WRITE_DELAY_US);
...
static int8_t write_config_file(const struct device *dev) {
    ...
    for (index = 0; index < cfg->feature->config_file_len; index += BMI270_WR_LEN) {
        ...
        ret = bmi270_reg_write_with_delay(dev, BMI270_REG_INIT_ADDR_0, addr_array, 2, BMI270_INTER_WRITE_DELAY_US);

        if (ret == 0) {
            ret = bmi270_reg_write_with_delay(
                dev, BMI270_REG_INIT_DATA, &cfg->feature->config_file[index], BMI270_WR_LEN, BMI270_INTER_WRITE_DELAY_US);
        }
    }
...
static int bmi270_sample_fetch(const struct device *dev, enum sensor_channel chan) {
    ...
    ret = bmi270_read_step_counter(&step_cnt);
    if (ret == 0) {
        data->step_counter = step_cnt;
    } else {
        data->step_counter = 0;
    }
...
static int bmi270_channel_get(const struct device *dev, enum sensor_channel chan, struct sensor_value *val) {
    struct bmi270_data *data = dev->data;
    ...
    } else if (chan == SENSOR_CHAN_STEP_CNT) {
        /* Store in val1 of the sensor_value struct */
        val->val1 = (int32_t)data->step_counter;
        val->val2 = 0;
    } else {
        return -ENOTSUP;
    }
...
#if defined(CONFIG_BMI270_PLUS_TRIGGER)
...
#endif /* CONFIG_BMI270_PLUS_TRIGGER */

static int bmi270_attr_set(
    const struct device *dev, enum sensor_channel chan, enum sensor_attribute attr, const struct sensor_value *val) {
    int ret = -ENOTSUP;
    ...
    if ((chan == SENSOR_CHAN_ACCEL_X) || (chan == SENSOR_CHAN_ACCEL_Y) || (chan == SENSOR_CHAN_ACCEL_Z) ||
        ...
        (chan == SENSOR_CHAN_GYRO_X) || (chan == SENSOR_CHAN_GYRO_Y) || (chan == SENSOR_CHAN_GYRO_Z) ||
...
static int bmi270_init(const struct device *dev) {
    ...
    adv_pwr_save = BMI270_SET_BITS_POS_0(adv_pwr_save, BMI270_PWR_CONF_ADV_PWR_SAVE, BMI270_PWR_CONF_ADV_PWR_SAVE_DIS);
...
#define BMI270_CONFIG_SPI(inst) .bus.spi = SPI_DT_SPEC_INST_GET(inst, BMI270_SPI_OPERATION, 0), .bus_io = &bmi270_bus_io_spi,
...
#define BMI270_CREATE_INST(inst)                                                                                      \
    ...
    static const struct bmi270_config bmi270_config_##inst = {                                                        \
        COND_CODE_1(DT_INST_ON_BUS(inst, spi), (BMI270_CONFIG_SPI(inst)), (BMI270_CONFIG_I2C(inst))).feature =        \
            BMI270_FEATURE(inst),                                                                                     \
        BMI270_CONFIG_INT(inst)};                                                                                     \
                                                                                                                      \
    SENSOR_DEVICE_DT_INST_DEFINE(                                                                                     \
        inst, bmi270_init, NULL, &bmi270_drv_##inst, &bmi270_config_##inst, POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY, \
        &bmi270_driver_api);
